...
void DW1000Class::begin(int ss, int irq, int rst) {
	// generous initial init/wake-up-idle delay
	delay(5);
	// start SPI
	dw1000SPI.begin();
	dw1000SPI.usingInterrupt(irq);
...
void DW1000Class::readBytes(byte cmd, word offset, byte data[], unsigned int n) {
	...
	for(i = 0; i < headerLen; i++) {
		dw1000SPI.transfer(header[i]);
	}
	for(i = 0; i < n; i++) {
		data[i] = dw1000SPI.transfer(JUNK);
	}
	delayMicroseconds(5);
